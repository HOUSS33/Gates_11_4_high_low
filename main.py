"""
========================================================================================
BOT DE SIGNAL LIVE (LOW/HIGH) : WEBSOCKET -> MACHINE A ETATS -> TELEGRAM
========================================================================================
Même infrastructure WebSocket/Telegram que les autres bots, moteur de signal
LOW/HIGH (1-18 / 19-36) "streak de répétition" (PAS chaos) :

- streak = nombre de tirages consécutifs du MÊME côté (Low ou High).
- Dès que streak atteint chaos_threshold, on parie sur le côté OPPOSÉ
  (celui "en retard").
- Mise fixe sur 4 paliers : 25, 50, 75, 100 DHS (pas de Fibonacci).
- Sur une perte : on change la cible pour chasser le côté qui vient de
...
# ==========================================================================
TELEGRAM_BOT_TOKEN = "8692293177:AAGimeIn24hEqw8FY2FtsJWjmhUW5KTx1wo"
...
# ==========================================================================
# 3. LA MACHINE A ETATS "EN LIGNE" — LOW/HIGH, PALIERS FIXES
# ==========================================================================
def get_range(n):
    if n == 0:
        return 0
    return 1 if 1 <= n <= 18 else 2  # 1 = Low (1-18), 2 = High (19-36)
...
class LiveSignalEngine:
    ...
    def process_spin(self, number):
        events = []
        c_rng = get_range(number)

        if self.p_rng is None:
            self.p_rng = c_rng
            return events
        ...
        if not self.is_betting:
            ...
            if self.streak_range == self.chaos_threshold - 1:
                label = "LOW (1-18)" if c_rng == 1 else "HIGH (19-36)"
                events.append(
                    f"⏳ <b>ALERTE PRÉCOCE</b> — {label} approche du seuil "
                    f"({self.streak_range}/{self.chaos_threshold}). Prépare-toi, "
                    f"le signal peut se déclencher au prochain spin."
                )

            if self.streak_range >= self.chaos_threshold:
                self.target_range = 1 if c_rng == 2 else 2  # côté opposé
                ...
                label = "LOW (1-18)" if self.target_range == 1 else "HIGH (19-36)"
                events.append(
        ...
        if actual_range == self.target_range:
            ...
            self.life_index += 1
            self.target_range = actual_range  # on chasse le côté qui vient de sortir
...
def handle_new_result(number, table_id):
    ...
    for msg in events:
        ...
        send_telegram_alert(msg)  # tout est relayé vers Telegram, sans filtre
    t2 = time.time()
...
def on_open(ws):
    ...
    if not _startup_alert_sent:
        send_telegram_alert(
            ...
            f"Telegram : tous les signaux sont relayés."
        )
...
if __name__ == "__main__":
    ...
    print(f"🎲 Bot LOW/HIGH démarré | Paliers : {engine.bet_ladder} | Exposition max/signal : {engine.actual_required_capital} DHS")
